...
3. Usage : translator_baidu.py <Mod filename>
...
appid = 20181103000229487
keyname = '4kc_8lc28HtqwXHWIuwb'
...
def translate(index, full):
    ...
    if not omit:
        salt = 0
        post = {'from' : 'en', 'to' : 'zh', 'q' : origin, 'appid' : appid, 'salt' : salt, 'sign' : hashlib.md5('{}{}{}{}'.format(appid, origin, salt, keyname).encode('utf-8')).hexdigest()}
        try:
            queue[index] = json.loads(urllib.request.urlopen(url,urllib.parse.urlencode(post).encode('utf-8')).read().decode('utf-8'))['trans_result'][0]['dst']
        except:
            info('thread : fatal : error when trying to translate through baidu.com, please check your network status.', cerr)
